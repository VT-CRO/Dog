...
from motor_config import motor_config  # updated motor_config that uses Arduino serial

# Initialize motor interface
Dingo = motor_config(port='/dev/ttyAMA0')  # adjust port if needed
...
servo_options = {"fr", "fl", "br", "bl", "all", "relax"}

# Calibration offsets per leg
offsets = np.array([
    [70, 107, 115, 64],  # hip
    [35, 10, 15, 22],    # upper
    [16, 27, 35, 14]     # lower
...
servo_name = sys.argv[1] if len(sys.argv) > 1 and sys.argv[1] in servo_options else "all"
servo_move = sys.argv[2] if len(sys.argv) > 2 and sys.argv[2] in position_dict else "cal"
pos = position_dict[servo_move]

if servo_name != "relax":
    print(f'DINGO: Moving motor "{servo_name}" to "{servo_move}" position.')
else:
    print("DINGO: Relaxing all motors.")

# Helper function
def move_leg(leg_index, joint_ids):
    for i in range(3):
        servo_id = joint_ids[i]
        angle = offsets[i, leg_index] + pos[i]
        Dingo.moveAbsAngle(servo_id, angle)

# Execute
if servo_name in ["fr", "all"]:
    move_leg(0, [Dingo.front_right_hip, Dingo.front_right_upper, Dingo.front_right_lower])
if servo_name in ["fl", "all"]:
    move_leg(1, [Dingo.front_left_hip, Dingo.front_left_upper, Dingo.front_left_lower])
if servo_name in ["br", "all"]:
    move_leg(2, [Dingo.back_right_hip, Dingo.back_right_upper, Dingo.back_right_lower])
if servo_name in ["bl", "all"]:
    move_leg(3, [Dingo.back_left_hip, Dingo.back_left_upper, Dingo.back_left_lower])
if servo_name == "relax":
...
# from ServoCalibrationDefinitionArduino import motor_config
